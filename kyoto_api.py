...
class KyotoTennis:
    ...
    def start(self, place: int, date: datetime.datetime):
        ...
        self.driver.find_element(
            "xpath", '//input[@src="../image/shisetu_kensaku.gif"]'
        ).click()
        # Search place
        if place == 0:
            place_name = "西院"
        ...
        self.driver.find_element("name", "txt_keyword").send_keys(place_name)
        self.driver.find_element("name", "btn_shortcut").click()
        self.driver.find_element(
            "xpath", '//input[@src="../image/yoyaku_s.jpg"]'
        ).click()
        # Change Date
        ymd = date.strftime("%Y%m%d")
        this_month = datetime.date.today().month
        if (datetime.date.today().month + 1) % 12 == date.month % 12:
            self.driver.find_element("xpath", '//img[@alt="翌月表示"]').click()
        self.driver.find_element(
            "xpath", f'//a[@href="javascript:set_data({ymd})"]'
        ).click()
    ...
    def login(self):
        self.driver.find_element("name", "txt_usr_cd").send_keys(KYOTO_ID)
        self.driver.find_element("name", "txt_pass").send_keys(KYOTO_PW)
        self.driver.find_element("name", "btn_ok").click()

    def get_avail(
        self, place: int, date: datetime.datetime
        ...
        time.sleep(10)
        rows = self.driver.find_elements("class name", "clsShisetuTitleOneDay")
        for row in rows:
            if row.text == "テニスコート":
                tennis = row.find_element("xpath", "..")
                break
        komas = tennis.find_elements("class name", "clsKoma")
        t = 8
        avail_list = []
        for koma in komas:
            dt = int(koma.get_attribute("colspan")) // 4
            avail = koma.find_element("tag name", "img").get_attribute("alt")
            if "予約可能" in avail:
    ...
    def reserve(self, place: int, date: datetime.datetime):
        ...
        for row in rows:
            if row.text == "テニスコート":
                ...
                break
        t = 8
        for koma in tennis.find_elements("class name", "clsKoma"):
            t += int(koma.get_attribute("colspan")) // 4
            if date.hour < t:
                avail = koma.find_element("tag name", "img")
                break
        else:
        ...
        for c in self.driver.find_elements("class name", "clsKomaLarge"):
            if "予約可能" in c.get_attribute("alt"):
                c.click()
                break
        self.driver.find_element("name", "btn_close").click()
        self.driver.switch_to.window(self.driver.window_handles[-1])
        self.driver.switch_to.frame("MainFrame")
        self.driver.find_element("name", "btn_ok").click()
        self.login()
        self.driver.find_element("name", "btn_next").click()
        self.driver.find_element("name", "btn_toroku").click()
        self.driver.find_element("name", "btn_cmd").click()
